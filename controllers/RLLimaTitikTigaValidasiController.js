...
export const getDataRLLimaTitikTigaValidasi = (req, res) => {
  if (!req.query.rsId) {
...
export const insertDataRLLimaTitikTigaValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlLimaTitikTigaValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlLimaTitikTigaValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLLimaTitikTigaValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
