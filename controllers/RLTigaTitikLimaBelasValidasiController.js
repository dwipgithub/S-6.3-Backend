import { rlTigaTitikLimaBelasValidasi } from "../models/RLTigaTitikLimaBelasValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikLimaBelasValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikLimaBelasValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikLimaBelasValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikLimaBelasValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikLimaBelasValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikLimaBelasValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikLimaBelasValidasi.update(updateData, {
      where: {
        id: req.params.id,
      },
    });
