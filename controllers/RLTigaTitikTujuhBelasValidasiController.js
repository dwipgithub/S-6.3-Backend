import { rlTigaTitikTujuhBelasValidasi } from "../models/RLTigaTitikTujuhBelasValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikTujuhBelasValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikTujuhBelasValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikTujuhBelasValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikTujuhBelasValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikTujuhBelasValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikTujuhBelasValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikTujuhBelasValidasi.update(updateData, {
      where: {
        id: req.params.id,
      },
    });
