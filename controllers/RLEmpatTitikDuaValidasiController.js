...
export const insertDataRLEmpatTitikDuaValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlEmpatTitikDuaValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlEmpatTitikDuaValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLEmpatTitikDuaValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
...
export const getDataRLEmpatTitikDuaValidasi = async (req, res) => {
  if (!req.query.rsId) {
    return res.status(400).send({
      status: false,
