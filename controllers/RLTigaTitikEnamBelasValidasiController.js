import { rlTigaTitikEnamBelasValidasi } from "../models/RLTigaTitikEnamBelasValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikEnamBelasValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikEnamBelasValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikEnamBelasValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikEnamBelasValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikEnamBelasValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikEnamBelasValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikEnamBelasValidasi.update(updateData, {
      where: {
        id: req.params.id,
      },
    });
