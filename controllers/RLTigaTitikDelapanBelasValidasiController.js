import { rlTigaTitikDelapanBelasValidasi } from "../models/RLTigaTitikDelapanBelasValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikDelapanBelasValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikDelapanBelasValidasi = (req, res) => {
  const where = {};
  ...
  rlTigaTitikDelapanBelasValidasi
    .findAll({
      attributes: [
        "id",
        ["rs_id", "rsId"],
...
export const insertDataRLTigaTitikDelapanBelasValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikDelapanBelasValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikDelapanBelasValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikDelapanBelasValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikDelapanBelasValidasi.update(updateData, {
      where: {
        id: req.params.id,
      },
    });
