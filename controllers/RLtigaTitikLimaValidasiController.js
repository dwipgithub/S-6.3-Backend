import { rlTigaTitikLimaValidasi } from "../models/RLTigaTitikLimaValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikLimaValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikLimaValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikLimaValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikLimaValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikLimaValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikLimaValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikLimaValidasi.update(
      {
        status_validasi_id: req.body.statusValidasiId,
        catatan: req.body.catatan,
        user_id: req.user.id,
