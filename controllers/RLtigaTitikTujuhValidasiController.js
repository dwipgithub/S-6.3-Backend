import { rlTigaTitikTujuhValidasi } from "../models/RLTigaTitikTujuhValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikTujuhValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikTujuhValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikTujuhValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikTujuhValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikTujuhValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikTujuhValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikTujuhValidasi.update(
      {
        status_validasi_id: req.body.statusValidasiId,
        catatan: req.body.catatan,
        user_id: req.user.id,
