import { rlTigaTitikSebelasValidasi } from "../models/RLTigaTitikSebelasValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikSebelasValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikSebelasValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikSebelasValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikSebelasValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikSebelasValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikSebelasValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikSebelasValidasi.update(updateData, {
      where: {
        id: req.params.id,
      },
    });
