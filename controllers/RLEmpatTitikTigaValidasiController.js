...
export const insertDataRLEmpatTitikTigaValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlEmpatTitikTigaValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlEmpatTitikTigaValidasi.create({
      rs_id: req.body.rsId,
...
export const getDataRLEmpatTitikTigaValidasi = async (req, res) => {
  if (!req.query.rsId) {
    return res.status(400).send({
      status: false,
...
export const updateDataRLEmpatTitikTigaValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
