import { rlTigaTitikEmpatValidasi } from "../models/RLTigaTitikEmpatValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikEmpatValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikEmpatValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikEmpatValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikEmpatValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikEmpatValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikEmpatValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikEmpatValidasi.update(
      {
        status_validasi_id: req.body.statusValidasiId,
        catatan: req.body.catatan,
        user_id: req.user.id,
