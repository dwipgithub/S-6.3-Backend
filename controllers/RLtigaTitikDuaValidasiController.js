import { rlTigaTitikDuaValidasi } from "../models/RLTigaTitikDuaValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikDuaValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikDuaValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikDuaValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikDuaValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikDuaValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikDuaValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikDuaValidasi.update(
      {
        status_validasi_id: req.body.statusValidasiId,
        catatan: req.body.catatan,
        user_id: req.user.id,
