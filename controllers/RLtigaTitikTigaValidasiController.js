import { rlTigaTitikTigaValidasi } from "../models/RLTigaTitikTigaValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikTigaValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikTigaValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikTigaValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikTigaValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikTigaValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikTigaValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikTigaValidasi.update(
      {
        status_validasi_id: req.body.statusValidasiId,
        catatan: req.body.catatan,
        user_id: req.user.id,
