import { rlTigaTitikDelapanValidasi } from "../models/RLTigaTitikDelapanValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikDelapanValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikDelapanValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikDelapanValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikDelapanValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikDelapanValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikDelapanValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikDelapanValidasi.update(
      {
        status_validasi_id: req.body.statusValidasiId,
        catatan: req.body.catatan,
        user_id: req.user.id,
