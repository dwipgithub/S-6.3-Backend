import { rlTigaTitikTigaBelasValidasi } from "../models/RLTigaTitikTigaBelasValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikTigaBelasValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikTigaBelasValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikTigaBelasValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikTigaBelasValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikTigaBelasValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikTigaBelasValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikTigaBelasValidasi.update(updateData, {
      where: {
        id: req.params.id,
      },
    });
