import { rlTigaTitikSembilanValidasi } from "../models/RLTigaTitikSembilanValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikSembilanValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikSembilanValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikSembilanValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikSembilanValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlTigaTitikSembilanValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLTigaTitikSembilanValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikSembilanValidasi.update(
      {
        status_validasi_id: req.body.statusValidasiId,
        catatan: req.body.catatan,
        user_id: req.user.id,
