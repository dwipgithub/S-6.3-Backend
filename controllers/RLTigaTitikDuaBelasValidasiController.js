import { rlTigaTitikDuaBelasValidasi } from "../models/RLTigaTitikDuaBelasValidasiModel.js";
import { statusValidasi } from "../models/RLTigaTitikDuaBelasValidasiModel.js";
import Joi from "joi";
import { Op } from "sequelize";

export const getDataRLTigaTitikDuaBelasValidasi = (req, res) => {
  const where = {};
...
export const insertDataRLTigaTitikDuaBelasValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlTigaTitikDuaBelasValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
...
export const updateDataRLTigaTitikDuaBelasValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
  ...
  try {
    ...
    await rlTigaTitikDuaBelasValidasi.update(updateData, {
      where: {
        id: req.params.id,
      },
    });
