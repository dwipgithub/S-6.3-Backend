...
export const getDataRLLimaTitikDuaValidasi = (req, res) => {
  if (!req.query.rsId) {
...
export const insertDataRLLimaTitikDuaValidasi = async (req, res) => {
  ...
  try {
    ...
    const dataExist = await rlLimaTitikDuaValidasi.findOne({
      where: {
        rs_id: req.body.rsId,
        periode: periode,
      },
    ...
    const result = await rlLimaTitikDuaValidasi.create({
      rs_id: req.body.rsId,
...
export const updateDataRLLimaTitikDuaValidasi = async (req, res) => {
  const schema = Joi.object({
    statusValidasiId: Joi.number().required(),
    catatan: Joi.string().when("statusValidasiId", {
      is: 1,
